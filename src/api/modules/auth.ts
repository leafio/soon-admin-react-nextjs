...
export const login = soon
  .API("/login")
  .POST<{ username: string; password: string }, { token: string; refreshToken: string }>()
export const refresh_token = soon
  .API("/auth/refresh")
  .POST<{ token: string }, { token: string; refreshToken: string }>()
